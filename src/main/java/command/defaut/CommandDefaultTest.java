package main.java.command.defaut;
...
import main.java.ClasseTest;
import main.java.QuestionTest;
import main.java.command.CommandTest;
...
public class CommandDefaultTest {
	...
	public CommandDefaultTest(ClassBotTest classBot) {
	...
	private void classe(User user, TextChannel textChannel, Guild guild, String command) {
		...
		if(args[1].equalsIgnoreCase("start")) {
			...
			if(args.length == 3) {
				...
				ClassBotTest.getClasses().add(classe);
			...
			for(Classe cl : ClassBotTest.getClasses()) {
				if(cl.getGuild().getId().equals(guild.getId()) && cl.getProf().getId().equals(user.getId()) && cl.getTextChannel() == textChannel) {
					...
					ClassBotTest.getClasses().remove(cl);
			...
			if(ClassBotTest.getMemberClasses().containsKey(member)) {
				...
				if(!ClassBotTest.getMemberClasses().get(member).getProf().getId().equals(user.getId())) {
					...
					ClassBotTest.getMemberClasses().get(member).removeUser(member.getUser());
					ClassBotTest.memberClasses.remove(member);
