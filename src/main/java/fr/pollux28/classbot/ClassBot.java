...
public class ClassBot implements Runnable {
	...
	public ClassBot() throws LoginException {
		/*
		 * jda = new JDABuilder(AccountType.BOT) .setToken(System.getenv("TOKEN"))
		 * .setActivity(Activity.watching("/classe help")) .build();
		 */
		jda = JDABuilder.create(GatewayIntent.getIntents(GatewayIntent.ALL_INTENTS)).setToken(System.getenv("TOKEN"))
				.setActivity(Activity.watching("/classe help")).build();
