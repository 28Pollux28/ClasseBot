	...
	private void classe(User user, TextChannel textChannel, Guild guild, String command) {
		...
		switch(args[1]) {
			...
			case "mute":
				...
				for(Classe cls : ClassBot.getClasses()) {
					if(cls.getGuild().getId().equals(guild.getId()) && cls.getProf().getId().equals(user.getId()) && cls.getTextChannel() == textChannel) {
						...
						textChannel.sendMessage(messageBuilder("La classe a été rendue muette", "Pour participer vocalement, demandez l'autorisation à "
			...
			default:
				...
				fieldContent = new String[]{"Ouvre cette interface","Permet de créer une classe si vous êtes prof. Vous devez être connecté dans un salon vocal et textuel oÃ¹ aucune classe n'est lancée,"
						+ " et préciser un nom de classe unique.", "Permet aux élèves de rejoindre la classe de leur professeur. Ils doivent se connecter dans le salon vocal oÃ¹ la classe est lancée.",
						...
						if(args.length >1) {
							...
							if(cl.getQuestionByMember(member) == null) {
								...
								sendPrivateMessage(member.getUser(), messageBuilder("Vous ne pouvez posez qu'une seule question", 
										"Afin de pouvoir poser une nouvelle question, merci de supprimer l'ancienne en cliquant sur la :x: située sous le message de question",1, fieldTitle, fieldContent, imgError));
							}
						}else {
							String[] fieldTitle = new String[]{"/question [question]"};
							String[] fieldContent = new String[]{"Vous permet de poser une question."};
