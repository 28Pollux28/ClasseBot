...
public class BotListener implements EventListener {
							...
							if (emoji.equals("âœ…")) {
								if (event.getMember().equals(cl.getGuild().getMember(cl.getProf()))) {
									if (q.getGuild().getMember(q.getUser()).getVoiceState().inVoiceChannel()
											&& q.getGuild().getMember(q.getUser()).getVoiceState().getChannel().getId()
													.equals(cl.getVoiceChannel().getId())) {
										q.setAnswering(true);
	...
	private void onVoiceJoin(GuildVoiceJoinEvent event) {
		...
		for (Classe cl : ClassBot.getClasses()) {
			if (guild.getId().equals(cl.getGuild().getId()) && cl.getVoiceChannel().getId().equals(vc.getId())
					&& !cl.getUsers().contains(user)) {
				if (guild.getSelfMember().canInteract(member)) {
					...
					sendPrivateMessage(user,
							messageBuilder("Vous ne pouvez pas vous connecter ici",
									"Un cours est en cours merci de ne pas le déranger. \n"
											+ "Si vous souhaitez le rejoindre :",
					...
					String[] fieldContent = { "Vous permet de rejoindre la classe de votre professeur." };
					sendPrivateMessage(user,
							messageBuilder("Merci de vous déconnecter",
									"Un cours est en cours merci de ne pas le déranger. \n"
											+ "Si vous souhaitez le rejoindre :",
									1, fieldTitle, fieldContent));
					return;
				}
			} else if (guild.getId().equals(cl.getGuild().getId()) && cl.getVoiceChannel().getId().equals(vc.getId())
					&& cl.getUsers().contains(user)) {
				if (guild.getSelfMember().canInteract(member)) {
					if (cl.isMuted()) {
						...
						sendPrivateMessage(user,
								messageBuilder("Vous avez automatiquement été rendu muet",
										"Pour participer vocalement, demander l'autorisation à "
												+ cl.getProf().getName() + " avec la commande",
										1, fieldTitle, fieldContent));
					} else {
						sendPrivateMessage(user, messageBuilder("Vous avez la parole !",
								"Profitez-en, cela ne durera pas forcément", 0, null, null));
					}
					...
					if (cl.isMuted()) {
						String[] fieldTitle = { "/question [sujet]" };
						String[] fieldContent = { "Vous permet de demander à prendre la parole." };
						sendPrivateMessage(user, messageBuilder("Vous n'avez pas automatiquement été rendu muet",
								"Merci de vous muter pour ne pas déranger le cours de la classe"
										+ "\nPour participer vocalement, demander l'autorisation à "
										+ cl.getProf().getName()
										+ " puis démutez vous quand vous recevrez un message après avoir utilisé la commande",
								1, fieldTitle, fieldContent));
						return;
					}
				}
